...
func _ready() -> void:
    ...
    SOCOMLog.log_event("Playtest: automation online")
...
func _step_move_order() -> void:
    if not issued_move:
        for unit in selection_handler.selection:
            ...
            unit.hold_mode = "off"
        issued_move = true
        SOCOMLog.log_event("Playtest: move order issued")
        step_timer = 0.0
        return
    if step_timer < 0.3:
        return
...
func _step_attack_move() -> void:
    if not issued_attack:
        ...
        for unit in selection_handler.selection:
            ...
            unit.hold_mode = "off"
        issued_attack = true
        SOCOMLog.log_event("Playtest: attack-move order issued")
        step_timer = 0.0
        return
    if step_timer < 0.3:
        return
...
func _step_hold() -> void:
    if not issued_hold:
        for unit in selection_handler.selection:
            ...
            unit.attack_move = false
        issued_hold = true
        SOCOMLog.log_event("Playtest: hold order issued")
        step_timer = 0.0
        return
    if step_timer < 0.2:
        return
...
func _step_end_run() -> void:
    if step_timer < 0.4:
        return
    SOCOMLog.log_event("Playtest: ending match")
...
func _advance_step(message: String) -> void:
    SOCOMLog.log_event("Playtest: %s" % message)
    step += 1
    step_timer = 0.0

func _fail(reason: String) -> void:
    SOCOMLog.log_event("Playtest failed: %s" % reason)
    if get_tree() != null:
