...
func _ready() -> void:
    ...
    for unit in get_tree().get_nodes_in_group("player_units") + get_tree().get_nodes_in_group("enemy_units"):
        SOCOMLog.log_event("Spawned Unit %d (role %s)" % [unit.id, unit.role])
    if playtest_active:
...
func _unhandled_input(event: InputEvent) -> void:
    ...
    if event is InputEventMouseButton and event.button_index == MOUSE_BUTTON_RIGHT and event.pressed:
        ...
        SOCOMLog.log_event("%s order issued to %d units" % [order_name, selection_handler.selection.size()])
        _record_timeline_event("%s order" % order_name, {
            "unit_count": selection_handler.selection.size(),
            "position": {"x": pos.x, "y": pos.y}
        })
        SOCOMLog.log_telemetry("order_issued", {
            "order": order_name,
            "unit_count": selection_handler.selection.size(),
            "pos_x": pos.x,
            "pos_y": pos.y
        ...
        if event.keycode == KEY_SPACE:
            ...
            SOCOMLog.log_event("Game paused" if get_tree().paused else "Game resumed")
            ...
            SOCOMLog.log_event("Formation mode set to %s" % mode)
...
func _end_run() -> void:
    ...
    SOCOMLog.log_event("State transition: Game -> AfterAction")
...
func _load_unit_archetypes() -> Dictionary:
    ...
    if not FileAccess.file_exists(path):
        SOCOMLog.log_event("Unit data missing: %s" % path)
        return {}
    var file: FileAccess = FileAccess.open(path, FileAccess.READ)
    var content: String = file.get_as_text()
    var parsed = JSON.parse_string(content)
    if typeof(parsed) != TYPE_DICTIONARY:
        SOCOMLog.log_event("Unit data invalid JSON: %s" % path)
        return {}
    return parsed

func _apply_unit_archetype(unit: Node, archetype_name: String) -> void:
    if not unit_archetypes.has(archetype_name):
        SOCOMLog.log_event("Unknown archetype: %s" % archetype_name)
        return
...
func _toggle_hold_mode() -> void:
    ...
    SOCOMLog.log_event("Hold mode set to %s" % mode_label)
...
func _handle_control_group_input(_event: InputEvent) -> void:
    ...
    if event.ctrl_pressed:
        ...
        SOCOMLog.log_event("Control group %d assigned (%d units)" % [group_id, selection_handler.selection.size()])
    ...
    SOCOMLog.log_event("Control group %d selected (%d units)" % [group_id, units.size()])
...
func award_xp(unit: Node, amount: int) -> void:
    ...
    SOCOMLog.log_event("Unit %d gained %d XP" % [unit.id, amount])
    if unit_roster.has(unit.id):
...
func record_flank_event(attacker: Node, target: Node) -> void:
    ...
    SOCOMLog.log_telemetry("flank_event", entry)
...
func _finalize_match_summary() -> void:
    ...
    SOCOMLog.dump_to_file("user://match_log.txt")
...
func _start_playtest_runner() -> void:
    ...
    SOCOMLog.log_event("Playtest: runner starting")
    add_child(runner)
