...
func _tick(delta: float) -> void:
    ...
    if pending_tactic != "":
        ...
        if comms_timer >= comms_delay:
            ...
            SOCOMLog.log_telemetry("ai_tactic_selected", {
    ...
    if _should_switch(next_tactic, sense):
        ...
        comms_timer = 0.0
        SOCOMLog.log_telemetry("ai_tactic_pending", {
            "fireteam_id": fireteam_id,
            "tactic": next_tactic,
            "reason": "switch",
...
func _sense() -> Dictionary:
    ...
    SOCOMLog.log_telemetry("ai_sense", {
        "fireteam_id": fireteam_id,
        "unit_count": units.size(),
    ...
    if enemy_flank:
        SOCOMLog.log_telemetry("ai_enemy_flank_detected", {
            "fireteam_id": fireteam_id,
            "enemy_count": player_units.size(),
            "centroid": centroid
        })
...
func _decide(sense: Dictionary) -> String:
    ...
    for key in scores.keys():
        ...
        if score > best_score:
            best_score = score
            best = key
    SOCOMLog.log_telemetry("ai_decide", {
        "fireteam_id": fireteam_id,
...
func _should_switch(next_tactic: String, sense: Dictionary) -> bool:
    ...
    if sense.get("heavy_suppression", false) or sense.get("heavy_losses", false) or sense.get("enemy_flank", false):
        SOCOMLog.log_telemetry("ai_tactic_abort", {
            "fireteam_id": fireteam_id,
            "tactic": current_tactic,
            "reason": "pressure",
...
func _evaluate(sense: Dictionary) -> void:
    ...
    if new_goal != commander_intent.get("goal", "hold"):
        ...
        SOCOMLog.log_telemetry("ai_intent_changed", {
            "fireteam_id": fireteam_id,
            "intent": new_goal
        })
    ...
    SOCOMLog.log_telemetry("ai_evaluate", evaluation)
...
func receive_help_request(team_id: int, unit_id: int, reason: String, metrics: Dictionary) -> void:
    if team_id != fireteam_id:
        return
    SOCOMLog.log_telemetry("ai_help_received", {
        "fireteam_id": fireteam_id,
        "unit_id": unit_id,
        "reason": reason,
        "metrics": metrics
